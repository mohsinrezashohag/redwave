...
| **CLNT-003** | Client billing rates are configured per client/product with a rate kind (product / tv_addon / hp_addon / bundle_bonus / spiff) and an effective date range, **in the client's currency**. **Bundles are admin-created and fully configurable** (added Meeting 3) — e.g. the RF Now $35 HP+TV bundle is one configured `bundle_bonus` rate, **not special-cased** in code; there is no hard-coded bundle logic. | **M**   |
...
| **EXP-013** | **Alert-vs-Warning validation (two levels).** A hard **Alert** blocks save until fixed (missing required info); a soft **Warning** (policy, e.g. over a soft cap) allows a deliberate "save anyway?" that **flags the item**. Alerts + warnings **aggregate to the report header with a count** so the approver sees flagged items before approving. | **M**   |
| **EXP-014** | **Client expense billing document.** A per-client document — **kilometres + food (meals), grouped by expense type, itemized per rep per day** — with **dynamic selection** of which reps / days / clients to include, rendered as a **gapless-numbered PDF** in the **client's currency** (§12). It contains **no receipts and no commission data** (#3) and is generated **separately** from the commission statement (attached separately when invoicing). The optional route-map image is **skipped** (not built).                       | **M**   |
