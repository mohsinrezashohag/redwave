...
- **Verified LOCAL** (backend **692 tests** incl. the pure engine + field-schema + save-blocking + the
  ...
  field → new items enforce it. **Still deferred:** the SA field-schema editor UI; report-as-folder aggregation
  on a report header; new categories beyond the 7-value `ExpenseCategory` enum (need an enum migration).
