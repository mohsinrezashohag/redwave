...
### 5a. Supabase GitHub App — DELIBERATELY UNINSTALLED (do not reinstall without reading this)

The **Supabase for GitHub** app was removed from the `redwavemarketing` GitHub account on 2026-09-07.
It had been installed ~3 months and was never doing anything useful here. Do not reinstall it casually —
it will fail the same way.

**What it does, and why neither half applies to us:**
- *"Runs your migrations when pull requests are merged"* — it looks for `supabase/migrations/` (Supabase
  CLI format). **Prisma owns migrations** in this repo (`backend/prisma/migrations/`), applied by an
  operator with `npm -w backend run prisma:deploy` and verified by reading the schema back. Supabase's app
  finds nothing to run and never has.
- *"Launches preview databases for new pull requests"* — each preview needs a Supabase **project slot**.
  Nothing in this codebase connects to a preview database: there is no `supabase/` directory and no
  `.github/workflows/`. The previews were empty databases nobody opened.

**Why it started failing.** The free plan caps a person at **2 active projects across all orgs they own or
administer**. Provisioning a preview validates every org admin/owner's headroom, and one member
(since removed) was already at their cap — so every merge went red in ~3 seconds. Removing that member
did not fix the displayed result, because a check result is a stored artifact of the run that produced it.

**Note the cap arithmetic if anyone reconsiders:** `redwave-prod` is 1 project against a limit of 2. One
preview branch fits; **two open PRs do not**. Reinstalling on the free plan buys a working integration with
zero margin, which fails intermittently — worse than failing consistently, because people learn to ignore
an unreliable red tick.

**Unaffected by the removal:** Vercel (a separate GitHub integration; production deploys were always
green), the migrations themselves, and the `redwave-prod` database. The app also held read/write access to
actions, checks, PRs and workflows across **all repositories** — so removing it was a security tidy-up too.

### 5b. Free-plan AUTO-PAUSE — the real operational risk

`redwave-prod` is on the Supabase **free plan**, which **pauses a project after a period of inactivity**.
This has already happened once (2026-09-07): the database became unreachable mid-session, TCP still
accepted connections on both pooler ports because the pooler host is shared infrastructure, and only the
Postgres handshake failed — a signature that is easy to misread as a network or credentials fault.

For a financial ledger heading to handover, a database that switches itself off after a quiet weekend is a
genuine risk. **Pro removes auto-pause.** Capacity is not the issue — usage is 39 MB of 500 MB, 1 MB of
5 GB egress, 0 of 50,000 MAU. It is the pausing that does not fit what this system is.
