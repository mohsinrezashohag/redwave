...
| **ID**       | **Requirement**                                                                                                                                             | **Pri** |
|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| **CLNT-001** | An Admin/Super Admin can create and edit clients (code, name, market CA/US, **billing currency**, whether the partner supplies MPU IDs, active flag). The four current program partners are **VF, RF Now, CTI (USD), and VF Business** (added Meeting 3). | **M**   |
| **CLNT-001a** | **Per-client billing currency (added Meeting 3).** Each client carries a `currency` (from the admin-configurable `currencies` set; USD + CAD primary, extensible). All of that client's billing rates and billing documents are interpreted/rendered in its currency; conversion to CAD for reconciliation follows the frozen stored-FX rule (BILL-011). The engine/rep-pay stream is unaffected — it stays CAD (#3). | **M**   |
...
| **CLNT-003** | Client billing rates are configured per client/product with a rate kind (product / tv_addon / hp_addon / bundle_bonus / spiff) and an effective date range, **in the client's currency**. **Bundles are admin-created and fully configurable** (added Meeting 3) — e.g. the RF Now $35 HP+TV bundle is one configured `bundle_bonus` rate, **not special-cased** in code; there is no hard-coded bundle logic. A `bundle_bonus` rate is client-wide (no product) and carries a **trigger product-type set** (`bundle_product_types`, ≥2 catalogue keys) — the product types a sale must all contain for the bundle to apply to its billed line total (see BILL-013). Distinct bundles are separate effective-dating scopes (keyed by their trigger), so one does not supersede another. | **M**   |
...
| **Screen / View**        | **Purpose & key UI elements**                                                                               |
|--------------------------|-------------------------------------------------------------------------------------------------------------|
| **Client List**          | All clients with status, market, and **currency**; add-client button.                                       |
| **Client Detail**        | Client info (incl. **billing currency**); Products tab (create/edit per-client products); Billing Rates tab (effective-dated rate rows, in the client's currency, incl. configurable bundles). |
...
clients (billing currency), currencies (config set), client_custom_fields, products, product_type_catalogue, client_billing_rates (client-currency-denominated).
...
| **SALE-001a** | **Internet is the mandatory base of a sale (added Meeting 3).** Every sale must include an internet activation (standard or greenfield); TV, Home Phone, Protection Plan, Mesh Extender, and Speed-attach are **add-ons that cannot be sold standalone**. Enforced **client- and server-side** (a standalone add-on is rejected 422). Add-on rate detection reuses `product_type_catalogue.behaviour` (base = tiered/greenfield). Whether an add-on pays **rep** commission is an admin-set flat rate — including **zero (bill-only)** — which never affects the internet-base rule. | **M**   |
...
| **Screen / View**    | **Purpose & key UI elements**                                                                                                                            |
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Sale Entry**       | Form: client dropdown → per-client products (internet base first; add-ons selectable only alongside an internet base — the form blocks a standalone add-on, mirrored server-side), customer + address, MPU ID, sale date, greenfield request checkbox. Live Sale ID preview. |
...
| **CLAW-009** | **Find candidates by Sale ID, customer name, address, or rep name (refined Meeting 3).** The clawback finder searches all four fields — because the same address can carry two reps, a match must resolve to a specific **sale + rep** before entry (the finder surfaces rep + address columns to disambiguate). Reuses the global search seam; no new permission. | **M**   |
...
| **Screen / View**  | **Purpose & key UI elements**                                                                                     |
|--------------------|-------------------------------------------------------------------------------------------------------------------|
| **Clawback Entry** | Find the sale by **Sale ID, customer name, address, or rep name** (rep + address columns disambiguate same-address reps) → select the paid item; auto-filled amount (editable only with permission); reason; reported date; save. |
...
**Report-as-folder capture (reworked in Meeting 3 — supersedes the earlier item-first model).** Expenses are captured in a **weekly report** — a folder that holds many expense line items and is **submitted as one unit** for approval. Approval is **per report** (approve, or **send back → rep fixes → resubmits**), not line by line. The model reference is the client's SAP Concur workflow. Retained from item-first: configurable per-type field sets, map-automated multi-stop kilometre logs, real receipt storage, a grouped/searchable list/export, and same-cycle payout derived from each item's own `expense_date`. Added by Meeting 3: the weekly-report folder + report-level submission/approval, a grouped searchable type picker, per-type field sets, a personal/do-not-reimburse toggle, custom tags (client + channel), an Alert-vs-Warning validation system aggregated to the report header, a per-client effective-dated km rate, receipts never shared with the client, and a separate client expense billing document.

### 11.1 Functional Requirements

| **ID**      | **Requirement**                                                                                                                                                                                                                                   | **Pri** |
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| **EXP-001** | **Weekly report = the submission unit (report-as-folder, reworked Meeting 3).** Any user (rep, manager, admin, partner) captures expense line items inside a **weekly report** (the business week — start day Sun/Mon confirmed at build); the **whole report is submitted as one unit** for approval, not line by line. `expense_reports` is the live submission entity (status Not Submitted / Submitted / Approved / Sent Back, week bounds, report-level approver, aggregated alert count). Each line item still carries its own `expense_date` (governing its pay cycle, EXP-009) and receipt. | **M**   |
| **EXP-001a** | **Report list + detail.** The report list shows name, week/date, **status** (Not Submitted / Submitted / Approved / Sent Back), **total**, and an **alert-flag count**, plus a running reimbursable total. Report detail has **Details / Expenses / Receipts** tabs and a **Submit** action. Submitting locks the report for approval; **send-back** reopens it for the rep to fix and resubmit. | **M**   |
| **EXP-002** | **Grouped, searchable expense-type picker** with a most-recently-used shortcut (e.g. Travel, Transportation, Meals, Office, kilometres, hotel, flight, rental, gas, other). The Super Admin can add new types/fields (with a per-type receipt rule) that appear in the picker. Config-driven, not hardcoded.                            | **M**   |
| **EXP-002a** | **Per-type field sets (fields differ by type).** Each expense type declares its own captured fields — e.g. *Mileage:* route/stops, distance, from/to, date, comment; *Meals:* amount, vendor name, city of purchase (recent-cities list), gratuity, attendees, receipt; *Hotel/Flight/Parking/etc.:* their own fields. **Common fields on every type:** currency, payment type, receipt status, the **personal-expense (do-not-reimburse) toggle** (EXP-012), and **custom tags** (client + channel). Field sets are config-driven (`expense_field_configs.fields`, SA-editable). The captured values are stored in `expense_items.field_values` (jsonb) as **METADATA ONLY** — they are shown to the approver/on the client doc but are **never summed into the reimbursable `amount`** (gratuity is a breakdown field, not an add-on; #1). *(New CATEGORIES beyond the built enum still need a schema migration; the FIELD schema is fully dynamic.)* | **M**   |
| **EXP-003** | A receipt upload is mandatory for every category except the kilometre log; enforced client- **and** server-side. Receipts upload to object storage (Supabase) and return an access-controlled URL. **Receipts are retained for tax and are NEVER included in any client-facing output** (they never appear on the client expense billing document — EXP-014).                                                | **M**   |
| **EXP-004** | **Kilometre log.** One km log per day **per rep**, with multiple stops; the user selects single trip (−30 km) or round trip (−60 km); billable km × rate is the amount, **computed server-side**. **The per-km rate is per client and effective-dated** (default $0.45), and is **two-stream** (#3): a rep-reimbursement rate (CAD) drives the pay-run amount, and a separate client-bill rate drives the client expense document — the two are never combined. When no per-client rate is configured the default applies. When a Maps key is configured, each stop is a Places autocomplete and the route distance is **re-derived server-side** from the coordinates (the client value is ignored); otherwise the user enters the total distance manually and the server falls back to it. **Trip semantics (confirmed):** a SINGLE trip is measured over the stops exactly as entered (stop 1 → … → stop N); a ROUND trip is measured as the **closed loop** — the return leg to the first stop is **appended automatically** when deriving distance, so the rep enters only the outbound stops (if the first stop was already re-entered as the literal last stop, it is not appended again). The −30/−60 deduction is unchanged by trip measurement. In the manual (no-Maps) fallback the rep types the full driven distance themselves. | **M**   |
| **EXP-005** | The “other” category provides a free description, date, amount, and receipt for ad-hoc costs (e.g. events).                                                                                                                                       | **M**   |
| **EXP-006** | A **submitted report** enters a Pending Approval queue; a Manager/Admin reviews the whole report and **approves, rejects, or sends it back** (rep fixes → resubmits). Approval is **per report**, not per line item. (A manager may still bulk-review across reports in their queue.)                                                                            | **M**   |
...
| **EXP-012** | **Personal / do-not-reimburse toggle.** Any line item can be flagged personal; a personal item is **excluded from the reimbursable total, the pay-run seam, and all client-facing output** (it is captured for completeness only).                       | **M**   |
| **EXP-013** | **Alert-vs-Warning validation (two levels).** A hard **Alert** blocks save until fixed (missing required info: amount, a required receipt, a required per-type field); a soft **Warning** (policy — over a category/field **soft cap**, or a **km trip the commute deduction zeroed** → $0 reimbursable) allows a deliberate "save anyway?" that **flags the item**. Validation is **DERIVED / stateless** (recomputed from the item + its schema on every read — no stored acknowledgement); approval is **not** gated by it (approver judgement, EXP-008). Alerts + warnings **aggregate to a count** so the approver sees flagged items before approving — *interim on the approvals/expense-list header (`GET /v1/expense-items/validation-summary`) until the report-as-folder rework moves it to the report header.* | **M**   |
| **EXP-014** | **Client expense billing document.** A per-client document — **kilometres + food (meals), grouped by expense type, itemized per rep per day** — with **dynamic selection** of which reps / days / clients to include, rendered as a **gapless-numbered PDF** (`CEXP-`) in the **client's currency** (§12), immutable (a regenerate supersedes). Sources **approved, non-personal** items only. **km is priced from the CLIENT-BILL km rate** (`km_rate_config.stream='client_bill'`, re-priced from the item's billable km — never the rep-reimbursement amount); a km item with **no configured client-bill rate for its date → 422**. **Food is billed native-currency** — a meal counts only when its entry currency equals the client's currency; a mismatch is **excluded** (surfaced, never converted). It contains **no receipts and no commission data** (#3) and is generated **separately** from the commission statement (attached separately when invoicing). The optional route-map image is **skipped** (not built).                       | **M**   |
...
| **Screen / View**   | **Purpose & key UI elements**                                                                                                                                                                          |
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Report List**     | The rep's weekly reports (folder-per-week): name / week / **status** (Not Submitted / Submitted / Approved / Sent Back) / **total** / **alert count** / running reimbursable total. Create-report + open-report actions. |
| **Report Detail**   | **Details / Expenses / Receipts** tabs + a **Submit** action. The Expenses tab lists this report's line items (add / edit / delete before submission); the header shows the aggregated alert/warning count. |
| **Add Expense**     | Inside a report: **grouped, searchable type picker** (recent-used) → **per-type field set** (km log: trip type + Places-autocomplete stops on a map with auto distance, or manual entry; meals: amount/vendor/city/gratuity/attendees/receipt; etc.) + common fields (currency, payment type, receipt, **personal toggle**, tags). Two-level validation (Alert hard-stop / Warning save-anyway). |
| **Approval Queue**  | Submitted **reports** (server-scoped); approve / reject / **send back**; Super-Admin edit after approval; flagged-item counts visible before approving.                                                  |
| **Expense Export**  | Grouping + PDF/Excel/CSV (per-item or grouped buckets); the **client expense billing document** (km + food, grouped by type, per rep/day, selectable, gapless-numbered PDF, client currency, no receipts); a server-recorded export with date/rep/client scope. |

### 11.4 Data Touchpoints

expense_reports (live weekly submission unit: status/week-bounds/approver/alert-aggregation), expense_items (line items with submitter/status/pay_period + `is_personal` + tags + stored-FX `{original_amount, original_currency, fx_rate, fx_rate_date, amount_cad}`), expense_km_logs, expense_km_stops, expense_field_configs (per-type field sets), km-rate config (per-client, effective-dated, two-stream), expense_exports, clients (billing currency), currencies (config set), pay_periods. External: Google Maps Directions (server) + Places (browser); Supabase Storage (receipts); FX rate source (Bank of Canada Valet API, env-gated with manual override). Rep reimbursement pays the frozen `amount_cad`; the client expense document renders in the client's currency.

## 12. Billing & Statements

Per-client, per-period output: the commission statement Excel (one line per customer), an optional one-line commission invoice, and — separately (BILL-012) — a **client expense billing document**. GST is excluded. Documents bill in the client's currency and freeze a CAD conversion at issue (BILL-011).
...
| **BILL-011** | **Multi-currency with a frozen FX snapshot (reversed Meeting 3 — supersedes single-currency CAD).** Each client bills in its own currency (`clients.currency`; e.g. CTI = USD, others = CAD) from an admin-configurable currency set (extensible beyond USD/CAD). **Reconciliation is in CAD:** at **document issue** the system captures that day's FX rate and freezes `{original_amount, original_currency, fx_rate, fx_rate_date, amount_cad}` on the issued statement/invoice — the rate + CAD value are **immutable and never re-converted** (a correction re-issues a new numbered doc capturing a fresh rate). Roll-ups/reconciliation read the frozen `amount_cad`; the client-facing figures render in the client's currency. `fx_rate` at high precision; `amount_cad` rounded 2 dp, half-up (the central rule). The FX rate source is env-gated (Bank of Canada Valet API) with manual override; preview shows an indicative (non-frozen) rate. | **M**   |
| **BILL-012** | **Split client billing: commission and expenses are SEPARATE documents (added Meeting 3).** Per client per period the system produces (a) the **commission** statement/invoice and (b) a distinct **client expense billing document** — they are separate PDFs, attached separately when invoicing. The expense document is gapless-numbered + immutable like statements (BILL-006/007), rendered in the client's currency, priced from the **client-bill** km rate + food only, grouped by expense type and itemized per rep per day with selectable reps/days/clients. km is re-priced from the client-bill km rate (422 if none configured for a km item's date); food is billed native-currency (mismatches excluded, never converted). It carries **no receipts and no commission data** (#3). See EXP-014. | **M**   |
| **BILL-013** | **Configured bundles are applied to statement/invoice line totals.** A `bundle_bonus` rate carries a configurable **trigger product-type set** (`client_billing_rates.bundle_product_types`, ≥2 catalogue keys, client-wide — no product). When a sale (customer/household) contains **all** of a bundle's trigger product types, the bundle amount is added **once** to that customer's statement line total (and the summary names the bundle), priced from the rate effective on the **sale_date** (CLNT-004/#10), in the **client's currency**, inside the frozen-FX total at issue (BILL-011/#12). It is **not special-cased** (any admin-configured bundle applies) and is **client-bill only** — no commission data is read or combined (#3). Base **products** must be priced (BILL-004 422); a bundle is additive/optional (a missing bundle rate is simply not applied). | **M**   |
...
| **Screen / View**        | **Purpose & key UI elements**                                                            |
|--------------------------|------------------------------------------------------------------------------------------|
| **Statement Generation** | Select client + period; preview one-line-per-customer rows (indicative FX); generate and download Excel (client currency, CAD frozen at issue). |
| **Invoice Generation**   | Generate one-line commission PDF for a client/period.                                    |
| **Expense Doc Generation** | Generate the **client expense billing document** (km + food, grouped by type, per rep/day; select reps/days/clients); gapless-numbered PDF in the client's currency, no receipts. |
| **Billing History**      | List of generated statements/invoices/expense documents with download.                   |

### 12.4 Data Touchpoints

client_statements, client_statement_lines, client_invoices, the client expense billing document, sales, clients (billing currency), currencies (config set), expense_items (client-bill km + food), pay_periods. Each billing artifact carries the frozen stored-FX fieldset `{original_amount, original_currency, fx_rate, fx_rate_date, amount_cad}` captured at issue.
...
| **RPT-015** | **On-demand report exports.** A permitted user generates and downloads a report file (CSV/Excel/PDF) from `/reports/exports`: **business summary** (period scalars; `reports:business`), **leaderboard standings** (counts only; `reports:view`), **pay-run summary** (a period's run lines; `payrun:export`), **expense summary** (items by date range; `expenses:export`). Data comes from the EXISTING scope-enforced reads (rep = self, manager = roster); **no new permission** — the per-type key is enforced server-side. Each export is **recorded first** in `report_exports` (who/what/when + the caller's rep scope, like `expense_exports`) with an audit row; no record → no file. **Scheduled/recurring exports are deferred.** | **S**   |
...
| **Cross-period trends**            | `/reports/trends` — the Batch-4 trend charts (revenue/payout/margin, activations by product, revenue by client, tier distribution) with a periods-depth selector. Gated `reports:business` (same as the Business dashboard). |
| **Report exports**                 | `/reports/exports` — pick a report type (only the types the caller may export) → scope (period / date range) → format (CSV/Excel/PDF) → generate + download; a "Recent exports" list shows the recorded who/what/when. Scheduled exports deferred (RPT-015). |
| **Chatbot**                        | Role-gated assistant widget answering from Redwave data.                                                                                                 |

### 14.5 Data Touchpoints

Read-only aggregation over: sales, sale_items, pay_run_lines, holdback_ledger, clawbacks, client_statements, expense_\*; plus sales_targets, notifications, notification_event_settings, chatbot_config, chatbot_conversations, chatbot_messages. One write-side entity: **report_exports** (RPT-015) — the who/what/when record of each on-demand report export (mirrors expense_exports; the file itself is client-generated).
