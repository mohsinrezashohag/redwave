...
export const TEMPLATES: TemplateDef[] = [
  ...
  {
    id: 'km_rates',
    label: 'Historical km rates',
    group: 'Master data',
    description: 'Back-dated per-km rates. Leave Client code blank for the global default.',
    fields: [
      f('client_code', 'Client code', 'VF', '', 'Client (by code). LEAVE BLANK for the global default rate.'),
      f('stream', 'Stream', 'rep', 'client_bill', "'rep' (reimbursed to the rep) or 'client_bill' (charged to the client). Separate streams — never combined.", true),
      f('rate_per_km', 'Rate per km', '0.45', '0.50', 'Rate per kilometre, up to 3 decimals.', true),
      f('effective_from', 'Effective from', '2026-01-01', '2026-01-01', 'Date the rate took effect. Back-dating is allowed here only.', true),
      f('effective_to', 'Effective to', '2026-06-30', '', 'Optional end date (open-ended if blank).'),
    ],
  },
  {
    id: 'commission_tiers',
    label: 'Historical tier schedules',
    group: 'Master data',
    description: 'Back-dated commission tier schedules — ONE ROW IS ONE WHOLE SCHEDULE.',
    fields: [
      f('client_code', 'Client code', '', 'VF', 'Client (by code). LEAVE BLANK for the global schedule every client falls back to.'),
      f('tiers', 'Tiers', '0-6:110|7-16:125|17-35:145|36+:160', '0-9:120|10+:150', 'The whole schedule in one cell: min-max:rate separated by |, with min+:rate for the open top bracket. Must cover every tally with no gaps or overlaps. Tier numbers are assigned by rate (highest = Tier 1).', true),
      f('effective_from', 'Effective from', '2026-01-01', '2026-01-01', 'Date the schedule took effect. Back-dating is allowed here only.', true),
      f('effective_to', 'Effective to', '2026-12-31', '', 'Optional end date (open-ended if blank).'),
    ],
  },
  {
    id: 'commission_flat_rates',
    label: 'Historical commission flat rates',
    group: 'Master data',
    description: 'Back-dated flat rates paid to the REP per activation (never a client billing rate).',
    fields: [
      f('client_code', 'Client code', '', 'VF', 'Client (by code). LEAVE BLANK for the global flat rate for this product type.'),
      f('product_type', 'Product type', 'tv', 'home_phone', 'Product-type catalogue key. Must already exist — an import never adds a type.', true),
      f('amount', 'Amount', '30.00', '30.00', 'Commission paid to the rep per activation (CAD, exact decimal).', true),
      f('effective_from', 'Effective from', '2026-01-01', '2026-01-01', 'Date the rate took effect. Back-dating is allowed here only.', true),
      f('effective_to', 'Effective to', '2026-12-31', '', 'Optional end date (open-ended if blank).'),
    ],
  },
...
export const KIND_TO_TEMPLATE: Record<string, string> = {
  ...
  km_rate: 'km_rates',
  commission_tier_schedule: 'commission_tiers',
  commission_flat_rate: 'commission_flat_rates',
  create_reps: 'reps',
